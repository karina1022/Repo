# 彭祥恩備註  
## 待完成事項  
### Jetson 組 
1.加上道路線偵測  
2.找更多行車紀錄器影片(mp4)

### STM 組:
1.找出為何換回兩塊stm32做傳輸而成功率下降的問題


### 大家:
1.寫期中報告講稿

## 已完成事項
### Jetson 組 :
~~1.事件編碼:Nano用uart傳STM32解碼結果~~    
~~2.接收編碼~~  
~~3.畫roi區域(盡量)(有 : 1 ; 沒有 : 0)~~  
~~4.統計:以現在部分uart可以直接先用gpio的high,low去取代。至少100張照片去訓練(roi目前先在畫面中除3，中間的部分是危險的)~~
~~5.測試影片~~
### STM 組:  
~~1.事件編碼傳送(發送端)~~   
~~2.接收事件編碼(接收端)~~  
~~4.紅外線做測試(像是50次中可以成功接收幾次=>算比例)(距離分別然後量測)~~  
~~5.從jetson接收編碼到輸出編碼，確定可以穩定接收~~  
~~6.顯示面板(模擬的就可以了)~~  
# 實驗室時間
週一 19:00-21:00<br>
週五 13:00-17:00<br>

# Deadline
6/10 期中專題報告

# 競賽規章
[競賽規章](./比賽/115年中國工程師學會學生分會工程論文競賽.pdf)<br>
[競賽網站](https://sites.google.com/nycu.edu.tw/cie2026)

[繳交摘要連結](https://docs.google.com/document/d/1MF3ZDuQ7WEb78oyQUcGclyubOcHs5ys2/edit?usp=sharing&ouid=100804695889335940668&rtpof=true&sd=true)

# 比賽  
1. [全國大專校院智慧創新暨跨域整合創作競賽](https://niicc.cilab.csie.ncu.edu.tw/)<br>
5/5-7/10 報名<br>
8/7 初賽文件截止<br>
11/1 決賽<br>

# 114-2 專題期末檔案
[書面共用連結](https://docs.google.com/document/d/1NJNoAbx0ZLG1K6YqtPzt2JPfd-iPmpdZNoQGL9qlgUw/edit?usp=sharing)

[海報共用連結](https://canva.link/8j4jp87wj2s3bix)